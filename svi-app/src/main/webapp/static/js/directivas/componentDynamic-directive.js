...
app.directive('componente', function($compile,$document, growl,$rootScope,$timeout,reporteService) {
    return {
    	restrict: 'E',
        ...
        link : function (scope, element, attrs){
        	...
        	scope.cambioSelect = function (parentValue, contador){
        		...
        		if(parentValue == null || parentValue == undefined){
        			if(paramParent.padres != null &&  paramParent.padres.length > 0){// en
																						// caso
																						// de
																						// que
																						// el
        			...
        			for(var i=0; i < paramsParandeSons.length; i++){// iterar
																	// los
																	// parametros
																	// dependientes
        				for(var j=0; j < scope.array.length; j++){// iterar
																	// lista de
																	// parametros
        					if(paramsParandeSons[i].cdParam === scope.array[j].cdParametro){// comparar
																							// el
																							// código
																							// por
																							// el
																							...
																							// va
																							// filtrar
        						var listCatValuesSon = scope.array[j].catValues;// Obtener
																				// los
																				// valores
																				// del
																				// catalogo
																				// del
																				// hijo
            					var nuevaListaValores = [];// inicializar la
															// lista donde se
															// van a filtrar los
															// valores
            					for(var k=0; k < listCatValuesSon.length; k++){// iterar
																				// el
																				// catalogo
																				// del
																				// hijo
            						var valoresSeparado = parentValue.toString();// se
																					// convierte
																					// a
																					// string
																					// el
																					// valor
            						var strValSplit = valoresSeparado.split(',');// se
																					// separan
																					// los
																					// valores
																					// por
																					...
																					// select
																					// multiple
            						for(var l=0; l < strValSplit.length; l++){// recorrero
																				// los
																				// valores
																				// separado
            							if(paramParent.padres != null &&  paramParent.padres.length > 0){// en
																											...
																											// de
																											// otro
            								for(var m=0; m < paramParent.padres.length; m++){// recorrer
																								// los
																								// padres
																								// del
																								// parametro
																								// filtro
            									// comparar los valores de la
												// lista
            									if(listCatValuesSon[k][paramsParandeSons[i].nbColumnaFiltro] == Number(strValSplit[l]) &&
            									   listCatValuesSon[k][paramParent.padres[m].nbColumnaFiltro]== scope.nameForm[paramParent.padres[m].cdParam].$modelValue
                        							...
                        							nuevaListaValores.push(listCatValuesSon[k]);// agregar
																								// valores
																								// filtrados
                        						}
                							}
            							}else{// caso en que el parametro no
												// depende de otro
            								if(listCatValuesSon[k][paramsParandeSons[i].nbColumnaFiltro] == Number(strValSplit[l])){// comparar
																																	// los
																																	// valors
                    							nuevaListaValores.push(listCatValuesSon[k]);// agregar
																							// los
            					...
            					scope.nameForm[scope.array[j].cdParametro].$setViewValue();// limpiar
																							// el
																							// contenido
																							// del
																							// parametro
																							// actual
            					$timeout(function (){// timeout para que
														// angular le de tiempo
														// limpiar el form
            						scope.nameForm.$setPristine();
            					}, true);
            					scope["listaValores"+j] = nuevaListaValores;// setear
																			// los
																			// nuevo
																			// valores
																			// de
        	...
        	scope.busquedaPrevia = function (idParametro, valores){
        		...
        		if(idParametro != undefined && valores != null && valores != ""){
        			reporteService.consultaPrevia(idParametro, valores).success(function (data){
        	...
        	function typeElementToCreate (parametro,count){
        		...
        		switch (parametro.componente.cdComponente) {
					...
					case "FHT":
						...
						break;
					case "SELEUNICSINBUS":
						scope["listaValores"+count] = parametro.catValues;// Generar
																			// un
																			// atributo
																			// dinamico
		...
		return elemReturn;// return angular element
        	...
        	function switcher(param){
				...
				// createAditionalProperties(param.parametrosPropAux);

        		let txValor = param.txValor;// obtener la cadena de txvalor
        		let txValorSeparado = param.txValor.split(',');// separar
																// cadena de
																// txvalor
        		let txValorUno = txValorSeparado[0].split('|');// separar el
																// label uno
        		let txValorDos = txValorSeparado[1].split('|');// separar el
																// label dos
        	...
        	function checkBox(param){
        		...
        		var elemReturn = `<div class="col-md-6">
        					<div class="form-group">
        						<div class="input-group">
        							<div class="checkbox">
                        				<input id="${param.cdParametro}" name="${param.cdParametro}" type="checkbox"
                        				class="styled" ng-model="${param.cdParametro}"/>
                        				<label for="${param.nbParametro}">${param.nbParametro}</label>
        							</div>
        						</div>
        					</div>
        				</div>`;
       		...
       		var elemReturn = `<div class="col-md-6">
							<div class="form-group">
								<div class="input-group">
									<div class="radio">
						  				<label>
						   				<input type="radio" name="${param.cdParametro}" value="${param.cdParametro}" 
        	...
        	function createAditionalProperties(listProperties){
        		...
        		if(listProperties != null && listProperties.length > 0){
        			angular.forEach(listProperties, function(value, key) {
        				/// /Validación cuando la propiedad no tiene valor
							// asociado
