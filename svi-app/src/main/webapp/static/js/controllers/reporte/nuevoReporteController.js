angular.module(appTeclo).controller('nuevoReporteController', function($scope, admiReporteService,
		$filter, $parse, growl, showAlert, $timeout,$compile) {

	$scope.reportesTaqVO = {
			url: "/reporteDinamicoConsulta"
	};
	...
	$scope.listaTemporalPorpiedadesChecs = [];
	...
	/* Variables modal testquery */
	$scope.columnasMetadata = [];
	
	/* Variables modal param tipo catalogo */
	...
	$scope.parametrosDependientes = {
									paramestrosTipoCatalogo : [],
	...
	/* Variables modal de dependencias */
	...
	/* Objecto que contendrá todos los catalogos usados en la vista */
		...
		if ($scope.altaRep.$invalid) {
			...
			admiReporteService.checkParams(scriptFullTmp).success(function(data) {
				...
				if($scope.params.length <= 0){
					/*
					 * ejecutar el query y obtener el metadata la lista params
					 * está vacio
					 */
					testQuerySinParametros($scope.params, scriptFullTmp);
					$scope.habilitarPaginacion = true;
					$scope.reportesTaqVO.tipoAgrupacion = $scope.tipoAgrupacion[0];// cuando
																					// sea
																					// un
																					// "select
																					// *
	...
	function testQuerySinParametros(lista, scriptFull){
			admiReporteService.crearTestQuery(lista, scriptFullTmp).success(function(data) {
				...
				if ($scope.resultadoTestQuery.stResult) {
					...
					/**
					 * Mandar a llamar funciones que consultan catalogos dentro
					 * de la configuración de los params
					 */
	...
	/**
	 * @descripcion Método para obtener objetos iguales dentro de dos listas
	 *              dadas
	 * @author Jorge Luis
	 */
	...
	/**
	 * @descripcion Método para obtener objetos diferentes dentro de dos listas
	 *              dadas
	 * @author Jorge Luis
	 */
	...
	/**
	 * Comprobar los parametros en caso de que sea la edición comprobar los
	 * nuevos y agregarlos la la lista original
	 */
		function comprobarEdicionQuery(paramasPeticion, listaParametros){
			if(listaParametros.length > 0 && paramasPeticion.length > 0){
				...
				paramInterseccion = intersectElements(aplicados, dePeticion);// sacar
																				// elemento
																				// iguales
				paramBorrar = diffElements(aplicados, dePeticion);// detectar
																	// elementos
																	// a borrar
				paramNuevos = diffElements(dePeticion, aplicados);// detectar
																	// elementos
																	// nuevos
				...
				if(paramBorrar.size > 0){
					...
					reordenarElementos(paramasPeticion);
				}else if(paramBorrar.size <= 0 && paramNuevos.size <= 0 && 
	...
	confirmaRemoverElementos = function (data){
		...
		mostrarConfigValidadcionCorrecto();
		comprobarDependenciasYParametros();
		reordenarElementos(data.listPeticion);
	};
	...
	confirmaEdicionScript = function (data){
		...
		/*
		 * Una vez que se eliminar los parámetros se agregan los nuevos
		 */
		var listTMP = [];
	...
	function mostrarConfigValidadcionCorrecto(){
		if($scope.params.length > 0){
			angular.forEach($scope.params, function(param, key) {
				...
				$scope.objCheck[param.cdParametro] = false;// construir el
															// objeto para las
															// opciones de
															// marcar todos
				$timeout(function() { // timeout para que angular le de tiempo
										// limpiar el form
	...
	$scope.filtroTipoComponente = function(idTipoParam, index, dataParam, confParams) {
		...
		if(dataParam.componente != undefined){
			dataParam.componente = undefined;
		}
		if (idTipoParam != undefined && idTipoParam != null) {
			$timeout(function() {
				var list = filtroTipoParamProp(idTipoParam.idTipoParametro);
				if(list.length <= 0){
					growl.warning('El tipo de parámetro no tiene componentes asociados', {ttl : 3000});
					$scope['relacionTipoParamsCompsFiltro' + index] = [];
				}else{
					$scope['relacionTipoParamsCompsFiltro' + index] = list;
				}
	...
	$scope.filtroCompPropiedad = function(componente, index, dataParam) {
		if (componente != undefined && componente != null) {
			...
			// growl.warning('Seleccione al menos un tipo de componente.', {ttl:
			// 3000});
	...
	$scope.agregarValoresProp = function(index, cdParamActual) {
		...
		$scope.parametroActualModal = cdParamActual; // asignar cdParametro
														// actual
		$scope.listaCompPropModal = [];

		if (listaValues.length > 0  && objectParamActual.componente.cdComponente != 'SWITCH') {
			for (var i = 0; i < listaValues.length; i++) {
				listaValues[i].checkedComp = false;
				listaValues[i].value = undefined;
			}
			$scope.modalConfigParam = true; // bandera para mostrar modal
			angular.copy(listaValues, $scope.listaCompPropModal); // clonar
																	// listas de
																	// propiedades
			...
			if ($scope.parametroActualModal != undefined) {
				...
				if (objectParamActual.propieades == undefined) { // preguntar
																	// si hay
																	// propiedades
																	// definidad
					objectParamActual.propieades = []; // definir lista de
														// propiedades
				} else {
					listaTmpPropiedadesAsociadas = objectParamActual.propieades; // asociar
																					// propiedades
																					// asociadas
	...
	/**
	 * Función para marcar una propiedad del componente
	 */
	...
	/**
	 * Función para agregar las propiedades checadas dentro de la lista de
	 * propiedades en el modal
	 */
	$scope.guadaTmpPropCompo = function(paramActual, objectPropScript, switcherProp) {
		...
		if ($scope.modalCompo.$invalid) {
					...
					if ($scope.listaCompPropModal[i].checkedComp) {
						if($scope.listaCompPropModal[i].cdPropiedad === 'QUERY' && objectPropScript != undefined){
							if(!objectPropScript['verificacionQuery'+paramActual]){
								...
								return;
							}else if($scope.propiedadTipoQuery.testQuery != undefined && !$scope.propiedadTipoQuery.testQuery.stResult){
								growl.warning('La ejecución del query no salió correctamente, favor de validar' , {ttl: 3000});
								return;
							}
			...
			/*
			 * }else{ growl.warning('Marque mínimo una propiedad' , {ttl:
			 * 3000}); }
			 */
	...
	/**
	 * Función para verificar si hay cambios en el form antes de cerra el modal
	 */
	$scope.checarSiHayCambio = function(paramActual) {
		if(paramActual == undefined){
		...
		if (lengthList > 0) {
			...
			if(propiedadesActuales != undefined && propiedadesActuales.length > 0){
				...
				if(angular.equals(listTMP, propiedadesActuales)){
					$scope.modalConfigParam = false; // si no hay cambio
														// ocultar el modal
				...
				$scope.modalConfigParam = false; // si no hay cambio ocultar
													// el modal
				$scope.objCheck[paramActual] = false;
				$scope.propiedadTipoQuery['requireQuery'+paramActual] = false;
				if($scope.propiedadTipoQuery.testQuery != undefined)$scope.propiedadTipoQuery.testQuery.mensaje = undefined;
			...
			$scope.modalConfigParam = false; // si no hay cambio ocultar el
												// modal
			$scope.objCheck[paramActual] = false;
			$scope.propiedadTipoQuery['requireQuery'+paramActual] = false;
			if($scope.propiedadTipoQuery.testQuery != undefined)$scope.propiedadTipoQuery.testQuery.mensaje = undefined;
	...
	$scope.paramTipoCatalogo = function (newValue,oldValue,currentParam){		
		if(currentParam.configParamTipoCatVO != undefined){
			showAlert.confirmacion("Las configuraciones realizadas se perderán", confirmaParamTipoCatalogo, currentParam, cancelaParamTipoCatalogo,currentParam);
			$timeout(function() {
	...
	$scope.comprobarParamTipCar = function (actualParam){
		if(actualParam.configParamTipoCatVO != undefined && $scope.cambiosEfectuadosTipCat[actualParam.cdParametro]){
			...
			if(!v1 || !v2 || !v3 || !v4 || !v5){
				showAlert.confirmacion("Los cambios sin aplicar se perderán", comprobarConfirmaPerder, actualParam, comprobarCancelaPerder);
			}else{
				$scope.idTablaSeleccionada = undefined;
				$scope.preseleccionarColumna = false;
	...
	$scope.setTablaSeleccionada = function (idTabla, paremetro, banderaPreselect){
		...
		if($scope.idTablaSeleccionada != undefined && paremetro.configParamTipoCatVO != undefined && !banderaPreselect){
			showAlert.confirmacion("Los cambios que haya realizado se perderán", confirmaSelectCambioCatalogo, param, cancelaSelectCambioCatalogo);
		}else{
	...
	function seleccionarCatalogo (objecto){// dentro del objeto viene idTabla y
											// el paremetro
		$scope.objeto = {iden: [],desc: [],restri: [],order: []};
		...
		if(objecto.banderaPreselect){
			...
			$scope.objectCatalogosParam = objecModified;// nuevo parámetro en el
														// modal actualizado
		}
	};
		...
		if(parametro != undefined){
			...
			admiReporteService.ejecutarQueryCatalogo(scriptCatExecute).success(function(data, status, headers, config) {
				...
				if(data.stResult){
					...
					/**
					 * clonar objeto para la validación de cambios al momento de
					 * la cancelación del modal
					 */
					$timeout(function() {
	...
	$scope.guardaReporteSinParams = function(altaRepform) {
		if (altaRepform.$invalid) {
			...
			var objectReportePetition = $scope.reportesTaqVO;
			objectReportePetition.parametros = $scope.params;
			console.log(objectReportePetition);
			console.log(JSON.stringify(objectReportePetition));
			admiReporteService.guadaReporte(objectReportePetition).success(function (data){
				if(data){
					growl.success('El reporte se guardó correctamente' , {ttl: 5000});
					resetForms();
				}
			}).error(function(data, status, headers, config) {
				growl.error('Ocurrió un error al guardar la información ' + data.message, {ttl: 4000});
			});
		}
	};
	...
	$scope.guardaReporte = function(listaParams, currentForm, altaRepform) {
		...
		if (currentForm.$invalid && altaRepform.$invalid) {
	...
	function resetForms(){
		...
		$scope.arrayParametros = [];
		$scope.paramConfig = false;
		$scope.propiedadTipoQuery ={};
		$scope.parametrosDependientes = {};
		$("#select2-tipoReporte-container").text("Seleccione una opción");
		$("#select2-tipoTitulo-container").text("Seleccione una opción");
		$("#select2-tipoAgrupacion-container").text("Seleccione una opción");
		$timeout(function (){// timeout para que angular le de tiempo limpiar
								// el form
			$scope.altaRep.$setPristine();
		}, true);
		$scope.columnasMetadata.length = 0;
		$scope.edicionQuery.editable = false;
		$scope.habilitarPaginacion = false;
	};
	...
	$scope.configurarParamDependientes = function (data, dependenciasActuales){	
		...
		if(dependenciasActuales != undefined){
	...
	$scope.cerarModalConfigDependencia = function(data,aplicados){
		if(!$scope.dependenciasAplicadas.cambioAplicado && aplicados != undefined){
			showAlert.confirmacion("Los cambios que no se hayan aplicado se perderán", 
	...
	$scope.changeSelectPadre = function (data, index, originalList){
		var dataSelected = data['paramPadre'+index];
		if(dataSelected == undefined){
			$scope.modeloAsociaciones['idPadreSelect'+index] = undefined;
		}
		// if(dataSelected != undefined)
			// aquitarElementoEnHijo(data, index, originalList);
		comprobarMismo(dataSelected,$scope.objectDependencias['paramHijo'+index],index);
		$scope.dependenciasAplicadas.cambioAplicado = false;
	};
	...
	/**
	 * Función para obtener el parametro actual a través de su cdParametro
	 */
	...
	// lista de pruebas rapido
		/*
		 * var obj = { cdParametro : "idComponente", nbParametro : "" }; var
		 * obj1 = { cdParametro : "idConcepto", nbParametro : "" };
		 * $scope.arrayParametros.push(obj); $scope.arrayParametros.push(obj1);
		 * $scope.params = angular.copy($scope.arrayParametros);
		 * $scope.paramConfig = true; // {marcarTodos : false};
		 * angular.forEach($scope.params, function(param, key) {
		 * //param.catalogoPropiedades = angular.copy($scope.propiedades);
		 * $scope.paramValue[key] = undefined;
		 * $scope.objCheck[param.cdParametro] = false; $timeout(function() {
		 * //timeout para que angular le de tiempo limpiar el form
		 * $scope.testQueryForm.$setPristine(); }, true); });
		 */
